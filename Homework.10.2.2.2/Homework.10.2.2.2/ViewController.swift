...
class ViewController: UIViewController {
    ...
    // Имя
    lazy var nameLabel: UILabel = createNameSurnameLable(text: "Имя", origin: CGPoint(x: mainImage.frame.minX + 30, y: mainImage.frame.maxY - 70))
    
    // Фамилию
    lazy var surnameLabel = createNameSurnameLable(text: "Фамилия", origin: CGPoint(x: nameLabel.frame.maxX + 5, y: nameLabel.frame.minY))
    
    // Возраст
    lazy var ageLabel: UILabel = createLabel(text: "25 лет", origin: CGPoint(x: nameLabel.frame.minX, y: nameLabel.frame.maxY + 2))
